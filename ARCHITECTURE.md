...
| Android 应用内更新：**走 PackageInstaller session，而不是把 content:// 丢给安装器** | 之前用 `FileProvider` + `ACTION_VIEW`，TV 上直接报「软件包似乎无效」，而这条路径没有任何反馈。现在优先走系统推荐的 `PackageInstaller` session：把 APK 字节流写进 session 再 commit，**完全不经过 content URI**（所以不存在「安装器读不到我们的文件」），并且用 `PendingIntent` + `BroadcastReceiver` 把系统的判定（`STATUS_FAILURE_*`：签名冲突 / 空间不足 / 被系统阻止 / 包无效…）通过 channel 回传给 Dart，设置页直接显示成人话；`STATUS_PENDING_USER_ACTION` 时必须自己拉起系统确认界面，否则会永远静默等待。老路径（`ACTION_VIEW` → 没有处理器则 `ACTION_INSTALL_PACKAGE`）保留为兜底。另外 `REQUEST_INSTALL_PACKAGES` 只是清单声明，Android 8+ 还要用户为「本应用」打开**安装未知应用**，设置页会明确提示并可一键跳转 |
