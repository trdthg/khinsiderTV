...
| TV 输入法：**默认用系统 IME**，自绘键盘是后备 | Android TV 自己的键盘方向键可用，所以 TV 上搜索框就是普通可编辑输入框（进入页面即聚焦 → 系统键盘弹出），与手机/桌面同一条路径。自绘的 `TvKeyboard` 保留为后备：搜索框左边有一个键盘按钮可以在两种方式之间切换（`TvKeyboardMode`，持久化，默认 `system`），从输入框按「左」（光标在开头时）就能聚焦到这个按钮 —— 万一某台设备/遥控器驱动不了系统 IME，按一下就能回到自绘键盘。自绘模式会把输入框设成 `readOnly`：只读字段不会建立 input connection，系统键盘就不会从面板后面冒出来。`TextField` 只在「获得焦点」时请求 IME，所以系统 IME 模式下额外提供「在输入框上按确定 → `TextInput.show`」，把被 Back 关掉的键盘叫回来 |
