...
| TV 专用键盘：**自绘、双层、网格走位** | 见 `lib/ui/search/tv_keyboard.dart`。字母层含 `⇧`（大写锁定，遥控器按一次换一个字母太痛苦）和数字；符号层含 `!@#$%&*()?`、括号标点与 `é è ü ö ä ñ ç`。切层的键固定在底行最后一格。方向键由键盘自己算（几何遍历在 960×540 上会卡在同一行），键位节点固定 10 列 × 5 行，切层后按当前层行长重新 clamp。手机/桌面照旧用系统输入法 |
