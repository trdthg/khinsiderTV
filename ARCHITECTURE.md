...
| 同步播放：**主机决定「播什么」，跟随端只管「怎么跟上」** | 协议在 `data/lan/playback_sync.dart`（纯函数，可单测），传输是 `WebSocket /kh/playback/ws`（状态下行 + ping/命令上行），信标 `pb:1` 让设备列表先知道谁能当主机。**负载不带音频 URL**：只给 albumId + 曲目序号 + 位置 + 主机时钟，跟随端用自己那套 API 取专辑 —— 队列/预取/专辑页/缓存全部照常，两台设备可以用不同音质。**校正分两档**：>120ms 直接 seek（不会自己消失），25~120ms 用播放速率微调（`setSpeed`，听不出来），<25ms 不动；每 500ms 校一次，时钟差每 2 秒 ping 一次取中位数。跟随端测量位置必须读 `positionStream`，**不能**读被节流到 250ms 的 `state.position`（噪声大于死区）。跟随端的传输键与通知栏按钮**转发给主机**执行，绝不本地执行（否则两边打架）。**远程控制**：主机可 `POST /kh/playback/follow`（带自己的 id/端口）让列表里每台设备主动跟随，接收端只信**连接来源地址**、不信负载里的地址；`onFollowRequest` 在 `PlaybackSyncController.build()` 里就装好（LAN 服务可能稍后才起来，所以同时监听 `lanControllerProvider`）。跟随失败必须**可见**：错误写进 `state.error` 并在设置页显示，绝不空 catch。**位置必须外推**：just_audio 的 `positionStream` 约 200ms 才报一次，拿旧采样和外推到当前的目标比会凭空造出 200ms 偏差，于是不停 seek→卡顿；`livePosition()` 负责外推（暂停/时间倒流不外推）。**seek 是重手段**：阈值 250ms、两次之间至少 5s、seek 后 800ms 不校正位置，中间区间用速率（上限 5%）；同一 tick 有 `_correcting` 重入保护；速率只在变化 >0.2% 时才下发。**跟随人数是推送的**：`onFollowerCountChanged` 在接入/断开时回调刷新界面，绝不在开启共享时读一次快照（那一刻必然为 0）。**跟随端不能深跳**：它自己拉流，seek 到未缓冲的深处会卡住（卡顿 + 偏差一路涨到一首歌的长度），所以采纳曲目时只在目标 ≤20s 时才 seek、否则从曲首开始，seek 阈值 1.5s、冷却 20s，其余全靠速率。**绝不在校正循环里 await 播放器调用**：just_audio 的 `play()` 直到播放结束才 complete，await 它会让循环永久卡住（`_correcting` 不释放 → 后续校正全被跳过 → 偏差固定在几十秒），所有 play/pause/seek/setSpeed 一律 `_fire()` 且整次校正 3 秒超时。**跟随端不得自行换歌**：所有改变队列/曲目的路径（包括 `snap.completed → _loopAlbum()` 这种内部自动行为，不只是按钮）都要过 `_isFollower` 守卫，否则跟随端自己跳回第 1 首而主机在第 9 首，偏差就永远几万 ms。跟随状态行同时显示**两边位置**（主机 X 秒 / 本机 Y 秒）：同一首歌差几万 ms 时，单看偏差无法区分是「目标算错」还是「本机位置读错」 |
| 设置项：**「能聚焦」和「有动作」是两件事** | `SettingsRow` **永远**是 `DpadTile`；`onSelect == null` 只意味着选中它什么也不做。曾经 null 时返回纯文本，而行会在忙碌时（检查更新、下载、添加地址、测试连接、强制覆盖、清除缓存）把动作置空 —— 于是行在 tile/文本之间切换，元素树形状一变，焦点就跳给邻居。判据：**一个控件的可聚焦性不能随它的可用状态变化**；暂时不能用的按钮（`DpadIconButton`）保持可聚焦、只是变暗 + 空动作，只有值的行（版本号、占用）不变暗。`test/settings_focus_test.dart` 锁住「落点顺序 = 画出来的顺序」以及忙碌/插入行时焦点不动 |
