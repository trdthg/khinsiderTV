...
| Android 应用内更新：**下载 → 交给系统安装器**，还差一个系统开关 | `REQUEST_INSTALL_PACKAGES` 只是清单声明；Android 8+ 还要用户为「本应用」打开**安装未知应用**，否则安装界面一闪而过、什么都不做 —— 这正是「更新失败但不知道为什么」的原因。所以新增 `canInstallPackages` / `openInstallSettings` 两个 channel 方法，设置页会直接说「系统还没有允许本应用安装应用」并给一个跳转按钮；安装被拒绝时**保留已下载的安装包**（阶段仍是 `downloaded`，只显示 `installError`），重试安装不会重新下载 40MB。安装前先下 `Intent.ACTION_VIEW` 探测，没有处理器就退回 `ACTION_INSTALL_PACKAGE`（纯 Android TV 镜像可能只认后者）；新版本下载前会删掉同扩展名的旧安装包 |
