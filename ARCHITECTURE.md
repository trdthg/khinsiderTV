...
| 局域网同步：**UDP 广播发现 + 明文 HTTP**，收藏只做并集合并 | 不用 mDNS：那需要每平台一个 DNS-SD 插件（Android NsdManager / Bonjour），而 `dart:io` 的 `RawDatagramSocket` + `HttpServer` 零依赖就够。发现：每台设备在固定端口 41234 上监听，向 `255.255.255.255` 和各网卡的 /24 定向广播地址发 `probe`，对方回 `pong`（带自己的 HTTP 端口，因为 HTTP 端口是系统临时分配的）；20 秒没消息就从未知设备列表里消失；广播被过滤的网络可以手填 IP（本质是向该地址单播 probe）。传输：`GET/POST /kh/favorites`，请求必须带 `x-khinsider: 1` 头，否则 403。**安全模型**：明文、无鉴权，前提是家庭局域网，且唯一能改变数据的操作是收藏并集合并（只增不删），不会删掉任何一边的收藏；两台设备都要开着应用。Android 侧为此把 `network_security_config` 的 `base-config` 打开 cleartext（对端 IP 是路由器分的，列不出来），iOS 加了 `NSLocalNetworkUsageDescription` |
