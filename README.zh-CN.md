...
目前我已经在 安卓/Macos/Windows/Chromecast 设备上面都测试了可以运行
...
这个软件包含两个部分, 一个是 gui, 一个是 khinsider 的 api, 通过解析 html 获取数据, 里面基本上就两个部分, 搜索, 获取专辑信息, 所以我想甚至可以提供多种数据源

- 我希望把所有的 音乐 以及 (搜索结果, 图片) 缓存 等直接保存在 Music 文件夹, 方便用户随意的拷贝清空,或者做任何事情
- 
...
Android 的签名密钥配置、CI 构建矩阵等细节见 [英文 README](README.md)。
