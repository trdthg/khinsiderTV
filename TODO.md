...
## AA. 已发布（v0.3.3）：让应用自己说清「连不上」到底是哪一半
...
## AC. 已发布（v0.3.5）：设置里每一行都能被焦点选中，而且次序不跳

用户反馈：「所有的设置选项都能选中，点不点击是另一件事，就是你的焦点不要跳来跳去的，很不稳定」。

- [x] 根因：`SettingsRow` 在 `onSelect == null` 时**返回纯文本而不是 `DpadTile`**（原本的理由是「别让焦点停在什么都不做的行上」）。
      偏偏有 6 处是**忙碌时把 `onSelect` 变成 null**（检查更新、下载、添加地址、测试连接、强制覆盖、清除缓存），
      于是这些行在 tile / 文本之间来回切换 —— 元素树形状变了，焦点就被扔给邻居，正好就是「跳来跳去」。
- [x] 现在 `SettingsRow` **永远是 `DpadTile`**：`onSelect == null` 只表示「选中它不做任何事」，
      行的可聚焦与否不再跟着动作的有无变化。不动作的行**不做变暗处理** —— 它们是「值」（版本号、占用大小），
      不是「暂时不能用的按钮」，`DpadIconButton` 那种变暗逻辑用在这里反而看不清。
- [x] 顺手确认 `DpadIconButton` 本来就是对的：禁用时仍是 `DpadTile`，只是 `Opacity(0.35)` + 空动作。
- [x] 测试 `test/settings_focus_test.dart`（4 个）：
      1. 用方向键从上往下走，**落点顺序 = 画出来的顺序**，一行都不漏；
      2. 只有值的行也能拿到焦点，按确认键不触发任何东西，下一行仍然只差一步；
      3. **检查开始时焦点不动**（就是原来会丢焦点的那个场景）；
      4. 列表上方插入新行（发现新版本）时焦点不动。

## AD. 已发布（v0.3.6）：同步播放 / 组环绕声（S8）

需求确认：**多设备同步播放**（不是真·声道拆分），**谁先开同步谁当主机**。

- [x] 协议 `lib/data/lan/playback_sync.dart`（纯逻辑，无 socket 无播放器）：
      `PlaybackSnapshot`（主机时钟 + 位置 + 专辑 id + 曲目序号）、`ClockSync`（ping/pong 估时钟差，取**中位数**）、
      `syncTarget` / `adviseSync`（偏差 >120ms 就 seek，25~120ms 之间用**播放速率微调**，<25ms 不动）。
- [x] **负载里不带音频 URL**：只给专辑 id + 曲目序号，跟随端用**自己的** API 层取专辑 —— 这样它的队列、
      预取、专辑页、缓存全都照常工作，两台设备甚至可以用不同的音质格式。
- [x] 传输：`WebSocket /kh/playback/ws`（状态下行、ping 与命令上行）+ `GET /kh/playback`；
      信标多了 `pb: 1` 表示「本机在共享播放」，所以设备列表不用先连就能显示「跟随它」。
      走的是同一个 `HttpServer`，鉴权头沿用 `x-khinsider`。
- [x] 主机：`PlaybackSyncController.startHosting()` 挂上 `playbackSnapshot` / `onPlaybackCommand`，
      并 `ref.listen` 播放状态 —— 位置每 tick 都会推一次（负载只有几百字节）。
- [x] 跟随端：连接 → 每 2 秒 ping 校时 → 每 500ms 校正一次；位置读的是 `positionStream` **而不是** UI 里
      被节流到 250ms 的 `state.position`（否则测量噪声比死区还大，校正等于白做）。
- [x] 跟随端的传输键（播放/暂停/下一首/上一首/停止/seek）**转发给主机**，包括通知栏和锁屏的按钮 ——
      在手机上按暂停暂停的是整个组，而不是和主机打架。
- [x] 局域网页面：顶部「把本机播放共享给其它设备」开关（开着时显示「N 台设备正在跟随」）+
      对支持共享的设备显示「跟随 <名字> 一起播放」+ 跟随中可「提前/延后 10 毫秒」微调、可停止跟随。
- [x] 测试 `test/playback_sync_test.dart`：25 条 —— 8 条同步数学（目标位置、延迟、暂停、seek/变速/不动三种
      处置）、5 条时钟同步（中位数抗抖动、样本不足不采信、过期失效）、3 条协议解析（脏包丢弃）、
      9 条**真实 socket 端到端**（连上先收状态、ping 有回音、命令到达主机、广播、恶意包不崩、断开被忘记、
      连不上有原因）。

## AE. 已发布（v0.3.7）：同步播放的修复 + 远程「让大家都跟随本机」

用户反馈：「follow 没有用；点 follow 之后某个设备 share 的滑块也会关闭；想要一键让所有其它设备跟随我，方便远程控制」。

- [x] **follow 会把本机的 sharing 状态弄乱**：`follow()` 只是把 `state` 重置，没有清掉 service 上的
      `hostingPlayback` / `playbackSnapshot` / `onPlaybackCommand`，所以界面显示「共享已关闭」而服务其实还在共享。
      现在 follow 会先走完整的 `stopHosting()`。语义上 v1 一台设备不能同时当主机又当跟随端，界面现在会**明确**
      显示「正在跟随 X」，不再是「什么都没发生」。
- [x] **跟随失败一律不可见**：`follow()` 的错误只写进 `state.error` 而界面从来不显示，`_applyTrack()` 的异常
      被空 catch 吞掉 —— 用户看到的就是「没有用」。现在局域网页面会显示红色「同步出问题：<原因>」，点一下可关掉。
- [x] **打开曲目失败后永不重试**：`_applied` 在**加载之前**就被赋值，于是那一首歌再也不会重试，跟随端整首歌静音。
      现在只有真正加载成功（并且 `PlayerController.error` 为空）才标记完成，失败则 3 秒后重试。
- [x] **新增远程控制：一键「让其它设备都跟随本机」**（主机侧开关旁边）。`POST /kh/playback/follow`
      带上本机 id/名字/端口，接收端用**连接的来源地址**（不是负载里的地址，防止被指到局域网里别的主机）回调
      `onFollowRequest` → 直接开始跟随。所以坐在沙发上用手机就能让电视跟过来，不用去碰电视遥控器。
- [x] 测试 +3（共 28 条）：被请求跟随的设备会收到正确的 id/端口与连接地址；没有同步控制器时返回 bad request 而不是崩；
      不带端口的请求被拒绝。
- [x] 待用户实测反馈：若仍无效，界面上的「同步出问题：…」会给出具体原因。

## AF. 已发布（v0.3.8）：修同步的卡顿与那个假的 -100~-200ms 偏差

用户反馈：「现在有用了，但显示 sync problem 的报错，而且跟随的设备除了有 -100~-200 的延迟外，还特别卡顿」。

- [x] **根因：位置采样是旧的**。just_audio 的 `positionStream` 大约每 **200ms** 才报一次位置，我拿这个最长 200ms
      前的旧值去和**外推到当前时刻**的目标比 —— 于是永远读出 -100~-200ms 的「偏差」，而这个偏差并不存在。
      校正于是不停的追它：超过阈值就 seek（每 500ms 一次）→ 这就是听到的**卡顿**。
      现在 `livePosition()` 会把采样值按「采样到现在的时间」外推（暂停时不外推；采样时间在未来时不外推），
      `adviseSync` 多了一个 `localSampledAtMillis` 参数。
- [x] **seek 变贵了，也更少了**：阈值 120ms → **250ms**；两次 seek 之间至少隔 **5 秒**；seek 之后 **800ms** 内
      不做位置校正（在途的 seek 报出来的位置没有意义）；`playAlbum` 之后那次立即 seek 失败不再当错误。
      中间那段（30~250ms）改用**速率**修正，上限从 3% 提到 **5%**（150ms 大约 3 秒追上）。
- [x] **同一个 tick 不叠加**：`_correcting` 重入保护 —— 定时器每 500ms 触发，而上一次校正可能还在 await 播放器，
      重叠的 seek 是能听出来的。
- [x] **同样的速率不再重复下发**：`setSpeed(1.0)` 每 500ms 写一次会让 just_audio 重新配置音频输出（有声音代价），
      现在只有变化超过 0.2% 才写。
- [x] **正在加载/缓冲时不校正**：`PlayerController.processing` 为真时只同步播放/暂停，不动位置 —— 避免缓冲期间
      偏差持续变大又不停 seek。
- [x] **报错会自己消失**：一次成功校正就清掉 `error`；跟随失败/加载失败的信息仍然保留可读（用户若再看到，请把
      那一行文字发出来）。
...
## AH. 已发布（v0.3.10）：卡顿与 -3万~-5万ms 的偏差

用户反馈：「同步还是卡，现在经常跳到 -3万 到 -5万ms」。

- [x] **-30~-50 秒的偏差 = 跟随端落后一整首歌的量级**，而它之所以追不上：跟随端**没有任何缓存就 seek 到曲目中段**。
      这台设备的音频是它自己拉的流，跳到尚未缓冲的位置意味着重新取流 → **卡住**（用户听到的卡顿），位置停住 →
      偏差一直读到 -3万~-5万；20 秒冷却到了再 seek 一次，于是「经常跳到」。
- [x] **采纳新曲目时只做浅跳**：`syncAdoptSeekLimit = 20s` —— 目标位置在 20 秒以内才 seek，否则**从曲首开始**
      （连续播放，下一首再对齐），因为断断续续的流比「这首从头放」更糟。
- [x] **seek 阈值 250ms → 1.5s、冷却 5s → 20s**：中间区间全部交给播放速率（≤5%，听不出来），
      让 seek 从「常规手段」变成「偶尔一次」。
- [x] 待用户实测：偏差应稳定在 1.5 秒以内，且很少出现 seek。

## AI. 已发布（v0.3.11）：真正的元凶 —— `await player.play()` 永不返回

用户反馈：「还是几万几万ms的延迟，没有缓冲，测试的是都缓冲完了的曲目」。

- [x] **根因**：just_audio 的 `play()` 返回的 Future 是**播放结束时**才 complete（不是开始播放时）。
      校正循环里 `await local.play()` 于是永远挂着 → `_correcting` 一直是 true → **之后所有校正（seek/速率）都被
      跳过**，偏差就永远停在那个数字上。这就是「几万ms 不变」。
- [x] **修法**：循环里所有播放器调用（play/pause/seek/setSpeed）改为 `_fire()` —— 不等待、错误吞掉（它们都是「请求」
      而不是下一步依赖的结果）；整次校正外面再加 3 秒超时，保证 `_correcting` 不可能卡住。
- [x] 同时撤销 v0.3.10 的「只在前 20 秒内才深跳」限制（用户用的是已缓冲完的曲目，那个限制只会让偏差长期停在几十秒），
      阈值 1.5s → **400ms**、冷却 20s → **6s**。
- [x] 教训：**播放器控制调用绝不能在同步循环里 await**；`play()` 的语义是「等播放结束」。

## AJ. 已发布（v0.3.12）：跟随端会自己偷偷换歌
...
## AK. 已发布（v0.3.13）：把两边的位置都显示出来

用户反馈：「还有，是一首」（同一首歌，但大多数时候仍显示几万 ms）。
...
- [x] ：自己发请求 → 写 （**换后缀**，不与
      just_audio 的  混淆）→ **关闭句柄** →  到成品，失败重试 5 次（Windows 不允许重命名打开中的文件）。
- [x] 播放时并行调用它（）：播放仍走 LockCachingAudioSource，另加一份自己收尾的下载，
      于是缓存里终于会出现真正的 。
- [x] **残留临时文件不再等于「正在下载」**：按**修改时间**判断（）——
      2 分钟没动过就是中断的残留，标记 ； /  都加了该状态。
- [x] 专辑页徽标： 显示「有被中断的下载留下的临时文件」+ 图标，**不再永远转圈**。
